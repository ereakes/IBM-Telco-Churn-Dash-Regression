---
title: "Telco Logistic Regression Script"
author: "Ewan Reakes"
date: "2025-07-11"
...
```{r}
# Loading selected datasets from IBM telco mock churn "https://tinyurl.com/3pf7rmua" ---------------------------
...
```{r}
# Removing redundant detail and IDs ---------------------------
service <- service %>%
  select(!c(1, 3, 4))
status <- status %>%
...
```{r}
# Creating matrix for use in the regression ---------------------------
X <- model.matrix(
...
# Scaling matrix to minimise bias and improve predictions
X_scaled <- X
...
standard_threshold <- ifelse(test_prob[, 1] > 0.5, 1, 0)

predicted_class_std <- ifelse(test_prob[, 1] > standard_threshold, 1, 0)

conf_matrix_std <- confusionMatrix(as.factor(predicted_class_std), as.factor(testing$`Churn Value`), positive = "1")
...
conf_matrix_opt <- confusionMatrix(as.factor(predicted_class_opt), as.factor(testing$`Churn Value`), positive = "1")

conf_matrix_opt
```

```{r}
# Exporting data for use in Power BI ---------------------------
# Performance metrics
metrics <- data.frame(
  Metric = c("AUC", "Sensitivity", "Specificity", "Precision", "F1"),
  Value  = c(auc_val,
             conf_matrix_opt$byClass["Sensitivity"],
             conf_matrix_opt$byClass["Specificity"],
             conf_matrix_opt$byClass["Precision"],
             conf_matrix_opt$byClass["F1"])
)
